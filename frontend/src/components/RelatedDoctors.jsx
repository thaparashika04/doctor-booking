...
const RelatedDoctors = ({ docId, specialization }) => {
  ...
  useEffect(() => {
    if (doctors.length > 0 && specialization) {
      const doctorsData = doctors.filter(doc => doc.specialization === specialization && doc._id !== docId)
      setRelDocs(doctorsData)
    }
  }, [doctors, docId, specialization])
   useEffect(() => {
    console.log("doctors array:", doctors)
    console.log("specialization prop:", specialization)
    console.log("docId prop:", docId)
    console.log("relDoc result:", relDoc)
  }, [doctors, specialization, docId, relDoc])
  return (
    <div className="flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10">
      ...
      <div className="w-full grid gap-6 pt-5 px-3 sm:px-0 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
